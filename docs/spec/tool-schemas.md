...
- **The unified `stop_code` is never the wire code Metrolinx actually accepts for a bus-only stop.** `Stop/Details`, `Stop/NextService`, `Stop/Destinations`, and `Schedule/Journey` all speak `LocationCode` — for a train or mixed stop that's identical to the unified `stop_code`, but a pure bus stop's unified code is its `PublicStopId`, a different value (e.g. Union Station Bus Terminal: unified `"102300"` vs. wire `"02300"`). Confirmed live 2026-07-21 (issue #35) for `Stop/Details` (204) and `Schedule/Journey` (empty `SchJourneys`); every tool/resource that accepts an exact stop code translates unified → wire via `resolveWireCode` (`src/normalize/search-stops.ts`) before calling upstream, and echoes the unified code back in any DTO field that carries one (issue #61). `plan_trip`'s fuzzy `from`/`to` entry point uses the equivalent `resolveStopByName`/`wireCode` field instead, since it never received an already-known exact code to translate.
...
- **Bus-only stop codes and the wire-code boundary — confirmed for two endpoints, defensively applied to two more (issue #61).** A bus-only stop's unified `stop_code` (its `PublicStopId`) is a different value from what `Stop/Details` and `Schedule/Journey` actually accept (`LocationCode`) — confirmed live 2026-07-21 (issue #35 verification): `Stop/Details/100132` (Square One's unified code) returned `ErrorCode 204`/`Stop: null` while `Stop/Details/00132` (its `LocationCode`) returned 200 with full data; `Schedule/Journey/.../100132/102816/...` silently returned `SchJourneys: []` while the `LocationCode` pair returned real services. `Stop/NextService` and `Stop/Destinations` were not independently re-verified live against a bus `PublicStopId` — no live credentials were available in the session that shipped the fix — but both are translated the same defensive way as the confirmed pair, on the reasoning that all four endpoints share the same `LocationCode` addressing scheme as `Stop/Details`. `get_stop_details`, `get_next_service`, `get_stop_destinations`, `plan_journey`, and the `gotransit://stops/{code}` resource all now translate the incoming unified `stop_code` to its wire `LocationCode` via `resolveWireCode` before calling upstream (see §1.4). A maintainer with a live key should extend `scripts/capture-fixtures.ts`'s bus-hub captures to independently confirm `Stop/NextService`/`Stop/Destinations` and update this note.
...
- **`get_fleet_consist` (`Fleet/Consist/All` and `Fleet/Consist/Engine/{EngineNumber}`) is Forbidden for a standard registered key — confirmed live (issue #3/#10, re-confirmed by the weekly smoke suite, issue #15).** Both calls return a tunneled `Metadata.ErrorCode: "403"`, surfaced as the tool's `upstream_error` with message `"Forbidden"`. This is the same undocumented Fleet-section authorization boundary as the `occupancy_percent` restriction below — see handoff-001 §2.7, which notes the *whole* `Fleet` section (not just Occupancy) sits behind separate authorization from the base Open Data API key. The tool implementation is correct (it relays the upstream rejection faithfully via the error taxonomy); this project's key has simply never had access. The smoke suite treats a Forbidden response here as the known, expected state rather than a hard failure, so it doesn't file a spurious `smoke-failure` issue every week. Whether elevated Fleet access can be requested from Metrolinx is unconfirmed — same open question as the Occupancy note below.
