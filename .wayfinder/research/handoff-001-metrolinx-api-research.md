...
Research date: 2026-07-16. All facts below were pulled directly from pages fetched under `api.openmetrolinx.com`, `gotransit.com`, `metrolinx.com`, and PDFs those sites link to. Every claim carries an inline citation to the exact page it came from. Where the official sites are silent, that is stated explicitly rather than inferred.
...
- **No API key parameter is documented anywhere.** None of the 34 endpoint Help pages list a `key`/`apikey`/`access_key` entry in their "URI Parameters" or "Body Parameters" tables, and no Help page or the registration page documents the exact query-string name, header name, or placement for the Access Key issued at registration. This is a genuine gap in the primary source — third-party unofficial sources (e.g. community Postman collections) suggest `?key=` as a query parameter, but that is **not confirmed by any page fetched from api.openmetrolinx.com** and should be verified empirically once a key is issued, not assumed. [(registration page, no key-usage documentation)](https://api.openmetrolinx.com/OpenDataAPI/Help/Registration/en)
