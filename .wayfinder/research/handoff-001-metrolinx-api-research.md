...
**Empirically confirmed (2026-07-18), against issue #11/PR #26 with a live key that otherwise works fine** (`Stop/*`, `Schedule/*`, `ServiceUpdate/*`, `ServiceataGlance/*`, `Gtfs/Feed/TripUpdates`, `Gtfs/Feed/VehiclePosition` all succeed): `GET api/V1/Fleet/Occupancy/GtfsRT/Feed/VehiclePosition` returns a genuine transport-level **HTTP 401**, not the body-tunneled `Metadata.ErrorCode: "401"` documented as this API's universal auth-failure signal in §4. This is the same pattern already seen on `Fleet/Consist/*` (issue #3/#10: `Metadata.ErrorCode: "403"` there, tunneled rather than transport-level, but still a rejection) — the whole `Fleet` section, not just `Consist`, appears to sit behind separate authorization from the base Open Data API key, undocumented anywhere on the Help site. Whether this is a distinct product tier, a manual grant, or something else is unconfirmed; worth asking `OpenData.Program@metrolinx.com` directly if Fleet-Occupancy data (physical consist occupancy percentage) ever becomes a hard requirement rather than a nice-to-have. Until then, `get_vehicle_positions` sources its base data from the plain (accessible) `Gtfs/Feed/VehiclePosition` and treats `occupancy_percent` as best-effort — see `docs/spec/tool-schemas.md` §5.
